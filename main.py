...
def get_info(message: types.Message):
    if message.content_type == 'voice':
        ...
        context_messages[message.from_user.id].append(
            {"role": "assistant", "content": form_completion.choices[0].message.content})
...
def check_form(message: types.Message):
    ...
    if key_word in str(get_form.choices[0].message.content).lower():  # посмотреть ответ
        ...
        s2 = bot.send_message(message.chat.id, "Все ли верно?", reply_markup=markup_keyboard_accept)
...
def final_check_form(message: types.Message):
    ...
    if ok == 'Да':
        markup_keyboard_accept = types.ReplyKeyboardRemove()
        bot.send_message(message.chat.id,
                                 "Отлично! Начинаю подготовку твоей индивидуаьлной программы тренировок!",
                                 reply_markup=markup_keyboard_accept)
        context_messages[message.from_user.id].append(
            {"role": "assistant", "content": "Выдай полученную форму, без лишнего текста"})
        ...
        bot.register_next_step_handler(pre_program.choices[0].message.content, check_program)
    else:
...
def check_program(message: types.Message):
    ...
    if user_answer == 'Да':
        ...
        bot.register_next_step_handler(form.choices[0].message.content, check_program)
